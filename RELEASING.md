...
`github:mat3ra/prove#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/prove/releases/download/wip-<short-sha>/prove.tgz
```
...
```bash
npm run mat3ra:install -- prove wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/prove@<release-url>` with
